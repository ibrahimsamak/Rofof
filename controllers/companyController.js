...
exports.Driverlist = async (req, reply) => {
  try {
    const _Users = await Drivers.find({ supplier_id: req.params.id })
      .populate("supplier_id")
      .sort({ createAt: -1 })
      .select(["-token", "-password"]);
    const response = {
...
exports.userlistInfo = async (req, reply) => {
  try {
    const Advs = await Drivers.find({ supplier_id: req.params.id }).sort({
      createAt: -1,
    });
    const response = {
      items: Advs,
...
exports.block = async (req, reply) => {
  try {
    const user = await Drivers.findByIdAndUpdate(
      req.body._id,
      {
...
exports.getTop3Category = async (req, reply) => {
  try {
    ...
    await Order.find({
      $and: [{ StatusId: 4 }, { supplier_id: req.params.id }],
    })
      .populate({ path: "items.product_id", populate: { path: "product_id" } })
      .select("items")
      .exec(function (err, item) {
...
exports.importantCounters = async (req, reply) => {
  try {
    var _items = [];
    var allPostOrders = await Order.find({
      $and: [{ isRate: true }, { supplier_id: req.params.id }],
    }).count();
    ...
    var refillOrders = await Order.find({
      $and: [{ orderType: 2 }, { StatusId: 4 }, { supplier_id: req.params.id }],
    }).count();
    var canceledOrder = await Order.find({
      $and: [
    ...
    var basket = await Order.find({
      $and: [{ supplier_id: req.params.id }, { paymentType: 3 }],
    }).count();
...
exports.revenuPerYear = async (req, reply) => {
  try {
    ...
    var items = [];
    await Order.find({
      $and: [{ supplier_id: req.params.id }, { StatusId: 4 }],
    }).exec(function (err, result) {
      result.forEach((element) => {
        var month_number = new Date(element.createAt).getMonth();
        var month_name = monthNames[month_number];
...
exports.getOrders = async (req, reply) => {
  try {
    ...
    const total = await Order.find({
      $and: [{ orderType: { $ne: 3 } }, { supplier_id: req.params.id }],
    }).count();
...
exports.getOrdersSeacrh = async (req, reply) => {
  try {
    ...
    await Order.find({ supplier_id: supplier_id })
      .sort({ _id: -1 })
      .populate("driver_id")
      .populate("user_id")
      .populate({ path: "items.product_id", populate: { path: "product_id" } })
      ...
      .exec(function (err, item) {
        var result = _.filter(item, function (itm) {
          return (
            itm.user_id.full_name.indexOf(req.body.full_name) >= 0 ||
            itm.user_id.phone_number.indexOf(req.body.phone_number) >= 0
          );
        });
...
exports.getNewOrder = async (req, reply) => {
  try {
    const total = await Order.find({
      $and: [{ StatusId: 1 }, { supplier_id: req.params.id }],
    }).count();
    reply.send(total);
  } catch {
    throw boom.boomify(err);
...
exports.getSupplierPoint = async (req, reply) => {
  try {
    await Point.find({ supplier_id: req.params.id })
      .sort({ _id: -1 })
      .populate("supplier_id")
      .exec(function (err, item) {
        // if (err) return handleError(err);
...
exports.updatePoint = async (req, reply) => {
  try {
    const sp = await Point.findByIdAndUpdate(
      req.params.id,
      {
        point_price: req.body.point_price,
        supplier_id: req.body.supplier_id,
...
exports.login = async (req, reply) => {
  try {
    const Admins = await Supplier.findOne({
