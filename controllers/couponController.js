...
exports.updatecoupon = async (req, reply) => {
    try {
        ...
        const _coupon = await coupon.findByIdAndUpdate((req.params.id), {
            coupon: req.body.coupon,
            msg: req.body.msg,
            dt_from: req.body.dt_from,
            dt_to: req.body.dt_to,
...
exports.checkCoupon = async (req, reply) => {
    try {
        ...
        const sp = await coupon.findOne({ '$and': [{ "dt_from": { "$lte": getCurrentDateTime() } }, { "dt_to": { "$gte": getCurrentDateTime() } }, { coupon: req.body.coupon }] });
        if (sp) {
            const myReq = await Order.find({ '$and': [{ coupon: req.body.coupon }, { user_id: user_id }] }).count();
